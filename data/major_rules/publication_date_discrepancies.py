...
if __name__ == "__main__":
    ...
    # data for analysis
    bool_left = df["publication_date"].isna()
    bool_right = df["published_dt"].isna()
    print(len(df.loc[(~bool_left & ~bool_right)]))
    bool_ne = df["publication_date"] != df["published_dt"]
    df_out = df.loc[(~bool_left & ~bool_right & bool_ne), ["control_number", "citation", "publication_date", "published_dt", "major_rule_report", "url"]]
    df = df_out.rename(columns={"publication_date": "publication_date_fr", "published_dt": "publication_date_gao"})
    df.to_csv(p.parent / "publication_date_discrepancies.csv", index=False)
