...
```{r ceate graphing function}

graph <- function(interval){
  upper <- case_when(
    interval == 1 ~ max(tot_pages$total) + interval,
    interval != 1 ~ ceiling(max(tot_pages$total)/interval)*interval)
...
bar1 <- ggplot(tot_pages, aes(x = year, y = total)) +
  ...
  coord_cartesian(ylim = c(0, max(tot_pages$total) + 10000)) +
  theme_RSC +
...
# save bar2 as pdf
ggsave(here("charts", "output", "federal_register_pages_by_calendar_year.pdf"), plot = bar3, width = 12, height = 9, dpi = 300)
```
