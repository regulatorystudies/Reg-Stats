...
# x-axis breaks function
```{r}
# the 

xbreaks <- function(dataset, column_number){
  round(seq(min(dataset[column_number]), max(dataset[column_number]),
            by = (max(dataset[column_number])-min(dataset[column_number]))/11),0)
}
# this would be very easy to integrate into my charting function

x_spacing <- xbreaks(tot_pages, 1)
```
...
bar1 <- ggplot(tot_pages, aes(x = year, y = total)) +
  ...
  scale_x_continuous(breaks = xbreaks(tot_pages, 1), 
