...
bar1 <- ggplot(tot_pages, aes(x = year, y = total)) +
  geom_bar(stat = "identity", fill = GWblue, width = 0.8) +
  ...
  theme(
        ...
        plot.caption = element_text(hjust = 1, vjust = 0, margin = margin(t = 20, l = 6, unit = "pt"), size = 11)
        )
bar1
...
suppressWarnings({bar2 <- ggdraw() +
  draw_plot(bar1) +
 draw_image(logo, x = 0.09, y = 0.044, halign = 0, valign = 0, width = 0.2)
bar2})
