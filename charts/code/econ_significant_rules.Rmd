...
# rename columns
colnames(econ_sig_rules) <- c("year", "party", "rules", "withdrawn")

# get rid of withdrawn column
econ_sig_rules <- econ_sig_rules[ , c("year", "party", "rules")]

# get rid of bottom three rows
#econ_sig_rules <- econ_sig_rules[1:(nrow(econ_sig_rules) - 5), ]
...
bar1 <- ggplot(econ_sig_rules, aes(x = year, y = rules, fill = party, pattern = party)) +
  ...
  xlab("") +
  scale_y_continuous(breaks = seq(0, max(econ_sig_rules$rules) + 25, by = 25), expand = c(0, 0.05), limits = c(-2, max(econ_sig_rules$rules) + 25)) +
  scale_x_continuous(breaks = seq(min(econ_sig_rules$year), max(econ_sig_rules$year), by = 1), limits = c(min(econ_sig_rules$year), max(econ_sig_rules$year + 1)), expand = c(0, 0))+
  labs(caption = wrapped_caption) +
  theme(plot.caption = element_text(hjust = 1, margin = margin(t = 0, l = 36, b = 50, unit = "pt"))) +
  theme_RSC +
  coord_cartesian(ylim = c(0, max(econ_sig_rules$rules) + 25), xlim = c(1980, max(econ_sig_rules$year) + 1)) +
  # WHY DID YEAR HAVE TO BE SET TO MAX + 1
  theme(
...
# save bar2 as pdf
ggsave(here("charts", "output", "econ_significant_rules_published_by_presidential_year.pdf"), plot = bar2, width = 12, height = 9, dpi = 300)
```
