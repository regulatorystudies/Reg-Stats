...
graph <- function(interval){
  upper <- case_when(
    interval == 1 ~ max(econ_sig_rules$rules) + interval,
    interval != 1 ~ ceiling(max(econ_sig_rules$rules)/interval)*interval)
...
# save bar2 as pdf
ggsave(here("charts", "output", "econ_significant_rules_published_by_presidential_year.pdf"), plot = bar3, width = 12, height = 9, dpi = 300)
```
