...
```{r create graphing function}
graph <- function(interval){
  upper <- case_when(
    interval == 1 ~ max(cum_sig_long$econ_rules) + interval,
    interval != 1 ~ ceiling(max(cum_sig_long$econ_rules)/interval)*interval)
...
line1 <- ggplot(cum_sig_long, aes(x = months_in_office, y = econ_rules, color = president, group = president)) +
  ...
  ylab("Economically Significant Final Rules Published") +
...
# save line2 as pdf
ggsave(here("charts", "output", "cumulative_econ_significant_rules_by_presidential_month.pdf"), plot = line3, width = 12, height = 9, dpi = 300)
```
