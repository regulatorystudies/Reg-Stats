...
breaks3 <- c(seq(min(df2$year), max(df2$year), 3), 2021)
breaks5 <- c(seq(min(df2$year), max(df2$year), 5), 2021)
breaks6 <- c(seq(min(df2$year), max(df2$year), 6), 2021)
...
bar2 <-
  ...
  scale_y_continuous(breaks = seq(0, upper, by = 20000), expand = c(0, 0),
    ...
    scale_x_continuous(breaks = breaks6,
...
# save as pdf
ggsave(here("charts", "output", "cfr_pages_by_calendar_year_6.pdf"), plot = new_bar, width = 12, height = 9, dpi = 300)
```
