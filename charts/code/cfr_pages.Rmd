...
df2$year <- as.numeric(df2$year) #new

```

# x-axis breaks function
```{r}
# column_number must be the column on which the x-axis label will be based
...
x_spacing <- xbreaks(df2, 1) # testing
```
...
# replace scale_x_discrete w/ scale_x_continuous and add relevant commands
  scale_x_continuous(breaks = xbreaks(df2, 1),
                     limits = c(min(df2$year)-1, max(df2$year)+1),
                     expand = c(0,0)) + # only display even year
