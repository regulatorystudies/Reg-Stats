...
graph <- function(interval){
  upper <- case_when(
    interval == 1 ~ max(sig_fin_rules$rules) + interval,
    interval != 1 ~ ceiling(max(sig_fin_rules$rules)/interval)*interval)
...
bar1 <- ggplot(sig_fin_rules, aes(x = year, y = rules, fill = party, pattern = party)) +
  ...
  ylab("Significant Final Rules Published") +
  xlab("") +
...
# save bar2 as pdf
ggsave(here("charts", "output", "significant_rules_by_presidential_year.pdf"), plot = bar3, width = 12, height = 9, dpi = 300)
```
