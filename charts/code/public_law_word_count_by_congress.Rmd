...
pres_labels <- data.frame(
  ...
  line_position = c(93.5, 94.5, 96.5, 100.5, 102.5, 106.5, 110.5, 114.5, 116.5),
  # labels positioned at the midpoint of each presidential administration
  label_position = c(94, 95.5, 98.5, 101.5, 104.5, 108.5, 112.5, 115.5, 117.5),
  label = c("Ford", "Carter", "Reagan", "Bush 41", "Clinton", 
            "Bush 43", "Obama", "Trump 45", "Biden")
)

# create function
add_pres_annotations <- function(df, y_position, line_color = RSCdarkgray) {
...
bar1 <- ggplot(count_by_congress1, aes(x = congress, y = word_count)) +
  ...
  xlab("Congress") +
  add_pres_annotations(pres_labels, y_position = upper + 2e5) +
  scale_y_continuous(
