...
# generate line graph
line1 <- ggplot(cum_sig_long, aes(x = months_in_office, y = econ_rules, color = president, group = president,family = "avenir_lt_pro")) +
#         annotate(geom = "segment", x = 48, xend = 48, y = 0, yend = max(cum_sig_long$econ_rules) + 10, linetype = "dashed", linewidth = 0.50, color = RSCgray) +
 #        annotate(geom = "segment", x = min(cum_sig_long$months_in_office), xend = max(cum_sig_long$months_in_office), y = 0, yend = 0, linetype = "solid", linewidth = 1, color = RSCgray) +  
  ...
  scale_y_continuous(breaks = seq(0, max(cum_sig_long_NA$econ_rules) + 25, by = 25),
                     expand = c(0, 0),
                     limits = c(0, max(cum_sig_long_NA$econ_rules) + 25)
                     #labels = function(x) ifelse(x == 0, "0", x)
                     #labels = scales::label_number(drop0trailing=TRUE)
                     ) +
  scale_x_continuous(breaks = seq(2, max(cum_sig_long_NA$months_in_office), by = 2),
                     expand = c(0, 0),
                     limits = c(0, max(cum_sig_long_NA$months_in_office))
                     )
