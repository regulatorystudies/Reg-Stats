...
plot_data <- function(df, agency_acronym) {
  ...
  line1 <- ggplot(df, 
            ...
            ggtitle(paste(toupper(agency_acronym), "Rules Published in the Federal Register by Presidential Year")) +
...
for (agency_ in agencies) {
  df_agency <- rules_pub_long %>% filter(acronym == toupper(agency_))
  if ((nrow(df_agency) != 0) & (max(df_agency$rule_num) >= 10)) {
    plot <- plot_data(df_agency, agency_)
