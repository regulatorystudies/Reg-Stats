...
plot_data <- function(df, agency_acronym, interval) {
  ...
  line1 <- ggplot(df, 
            ...
            ggtitle(paste(agency_acronym, "Rules Published in the Federal Register by Presidential Year")) +
...
agencies <- c("DHS", "DOJ", "EPA", "SBA", "CFPB", "FCC", "FERC", "FTC", "SEC")

for (agency_ in agencies) {
  df_agency <- rules_pub_long %>% filter(acronym == agency_)
  plot <- plot_data(df_agency, agency_, 50)
  save_plot_png_pdf(plot, out, agency_)
