...
graph <- function(dataset, interval, max_add, title){
  ...
  scale_pattern_manual(values = c(Democratic = "stripe", Republican = "none")) +
  ggtitle(title) +
  ylab("Number of Economically Significant Final Rules") +
  xlab("") +
...
dhs_bar <- graph(dhs_rules1, 1, 1, "Economically Significant Final Rules Published by DHS each Presidential Year")
doe_bar <- graph(doe_rules1, 1, 1, "Economically Significant Final Rules Published by DOE each Presidential Year")
doi_bar <- graph(doi_rules1, 1, 0, "Economically Significant Final Rules Published by DOI each Presidential Year")
dol_bar <- graph(dol_rules1, 1, 1, "Economically Significant Final Rules Published by DOL each Presidential Year")
dot_bar <- graph(dot_rules1, 1, 0, "Economically Significant Final Rules Published by DOT each Presidential Year")
ed_bar  <- graph(ed_rules1, 1, 1, "Economically Significant Final Rules Published by ED each Presidential Year")
epa_bar <- graph(epa_rules1, 2, 1, "Economically Significant Final Rules Published by EPA each Presidential Year")
hhs_bar <- graph(hhs_rules1, 5, 3, "Economically Significant Final Rules Published by HHS each Presidential Year")
hud_bar <- graph(hud_rules1, 1, 1, "Economically Significant Final Rules Published by HUD each Presidential Year")
sba_bar <- graph(sba_rules1, 5, 4, "Economically Significant Final Rules Published by SBA each Presidential Year")
usda_bar <- graph(usda_rules1, 2, 1, "Economically Significant Final Rules Published by USDA each Presidential Year")
...
ggsave(here("charts", "output", "by_agency", 
            "doi_econ_significant_rules_by_presidential_year.pdf"), plot = doi_bar, width = 12, height = 9, dpi = 300) # check gray line at 0, not lining up and internet not forthcoming w/ solutions
...
ggsave(here("charts", "output", "by_agency", 
            "ed_econ_significant_rules_by_presidential_year.pdf"), plot = ed_bar, width = 12, height = 9, dpi = 300) # gray line a bit off, better now
...
ggsave(here("charts", "output", "by_agency", 
            "hud_econ_significant_rules_by_presidential_year.pdf"), plot = hud_bar, width = 12, height = 9, dpi = 300) # gray line way off, i think it has to do with the interval and max
