...
```{r}
 interval <- 1
 dataset <- dhs_rules1
 title <- "Economically Significant Final Rules Published by DHS Each Presidential Year"


 upper <- case_when(
    interval == 1 ~ max(dhs_rules1$rules) + interval,
    interval != 1 ~ ceiling(max(dataset$rules)/interval)*interval)
  ...
  coord_cartesian(ylim = c(0, upper),
                  xlim = c(min(dataset$year)-0.5,
                                max(dataset$year)+0.5),
                  expand = FALSE) + # expand does not work for some reason with scale_x_continuous, but adding this here and putting buffers on xlim seem to do the trick aesthetically.  Unable to figure out a solution to the contrasts & factor issue
  theme(
    plot.title = element_text(hjust = 0.5, margin = margin(b = 20, unit = "pt")),
    axis.text.x = element_text(angle = 65, hjust = 1, vjust = 1),
    axis.ticks.x = element_blank(),
...
```{r}
 dataset <- hhs_rules1
 title <- "Economically Significant Final Rules Published by HHS Each Presidential Year"
 interval <- 5
 ...
 upper <- ydynam(hhs_rules1, interval, 3)
