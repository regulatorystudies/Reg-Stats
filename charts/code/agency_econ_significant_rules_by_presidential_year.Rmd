...
graph <- function(dataset, interval, title){
  ...
  theme_RSC +
  coord_cartesian(ylim = c(0, upper + 0.1),
...
chart_n_save <- function(agency){
  ...
  ggsave(
    here("charts", "output", "by_agency", 
         paste0(tolower(agency), "_econ_significant_rules_by_presidential_year.pdf")
         ), 
    plot = get("bar"), 
    width = 12, 
    height = 9, 
  ...
  ggsave(
    here("charts", "output", "by_agency", 
         paste0(tolower(agency), "_econ_significant_rules_by_presidential_year.png")
         ), 
    device = "png", 
    plot = get("bar"), 
    width = 1200, 
...
print(bar)
# ƒ
# ^ FIGURE OUT WHAT'S GOING WRONG HERE, also still need to fix code for outputting individual charts
