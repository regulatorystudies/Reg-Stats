...
#plots = list(dhs_bar, doe_bar, doi_bar, dol_bar, dot_bar, ed_bar, epa_bar, hhs_bar, hud_bar, sba_bar, usda_bar)
...
# vector of agency acronyms
agencies = c("dhs", "doe", "doi", "dol", "dot", "ed", "epa", "hhs", "hud", "sba", "usda")

# loop over strings and use get to return the value of each named object
# e.g. get("dhs_bar") returns object dhs_bar
...
for(agency in agencies){
  ...
  rules1 <- clean(rules)
  title <- paste0("Economically Significant Final Rules Published by ", 
                  toupper(agency),
                  " Each Presidential Year")
  
  for (interval in intervals) {
    bar1 <- graph(rules1, interval, title)
    bar2 <-   bar2 <- ggdraw() + 
      draw_plot(bar1) + 
      draw_image(logo, x = 0.1, y = 0.08, halign = 0, valign = 0, width = 0.2)
  ...
  # save plots as png
  out = here("charts", "output", "by_agency2")
  if (!dir.exists(out)){
    dir.create(out, showWarnings = FALSE)
    }
