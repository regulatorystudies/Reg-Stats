...
# # WIP to load and name all CSV's at once - works to import data, but imported data uses long, tedious names from file names.  Not sure it is any more efficient that what is already used.
# file_names <- gsub(".csv", "", 
#                    list.files(path = here("data", "es_rules", "by_agency")))
# 
# names <- gsub(" ", "", paste("data_file_", sub("\\_.*","",file_names)))
# 
# for(i in file_names){
#   filepath <- file.path(path = here("data", "es_rules", "by_agency"), 
#                         paste(i, ".csv", sep=""))
...
```{r}
graph <- function(dataset, interval, max_add, width, title){
  ...
  theme(
    ...
    axis.line.x = element_line(linewidth = width, color = RSCgray),  # Change linewidth to eliminate white space b/t gray line and y-intercept line
...
```{r}
dhs_bar <- graph(dhs_rules1, 1, 1, 1, "Economically Significant Final Rules Published by DHS each Presidential Year")
doe_bar <- graph(doe_rules1, 1, 1, 1, "Economically Significant Final Rules Published by DOE each Presidential Year")
doi_bar <- graph(doi_rules1, 1, 0, 2.5, "Economically Significant Final Rules Published by DOI each Presidential Year")
dol_bar <- graph(dol_rules1, 1, 1, 1, "Economically Significant Final Rules Published by DOL each Presidential Year")
dot_bar <- graph(dot_rules1, 1, 0, 1, "Economically Significant Final Rules Published by DOT each Presidential Year")
ed_bar  <- graph(ed_rules1, 1, 1, 1.5, "Economically Significant Final Rules Published by ED each Presidential Year")
epa_bar <- graph(epa_rules1, 2, 1, 1, "Economically Significant Final Rules Published by EPA each Presidential Year")
hhs_bar <- graph(hhs_rules1, 5, 3, 1, "Economically Significant Final Rules Published by HHS each Presidential Year")
hud_bar <- graph(hud_rules1, 1, 1, 2.5, "Economically Significant Final Rules Published by HUD each Presidential Year")
sba_bar <- graph(sba_rules1, 5, 4, 1, "Economically Significant Final Rules Published by SBA each Presidential Year")
usda_bar <- graph(usda_rules1, 2, 1, 1, "Economically Significant Final Rules Published by USDA each Presidential Year")
