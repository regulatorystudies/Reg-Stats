...
graph <- function(interval){
  upper <- case_when(
    interval == 1 ~ max(rules_pub_long$rule_num) + interval,
    interval != 1 ~ ceiling(max(rules_pub_long$rule_num)/interval)*interval)
...
# save line2 as pdf
ggsave(here("charts", "output", "federal_register_rules_by_presidential_year.pdf"), plot = line3, width = 12, height = 9, dpi = 300)
```
