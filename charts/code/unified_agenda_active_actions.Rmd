...
df1_1 <- df1_1 %>% 
  ...
  mutate(season = factor(substr(yearseason, 6,7))) %>% 
  mutate(season2 = case_when(season == "" ~ "2012",
                             TRUE ~ season)) %>% 
...
bar2 <-
  ...
  scale_fill_manual(values = c(GWblue, GWbuff)) +

  ggtitle("Active Actions in the Unified Agenda") +
  ylab("Number of Actions \n") +
  xlab("Note: 'S' indicates Spring, and 'F' indicates Fall in the x-axis. Only one agenda was published in 2012. \n") +
...
# charting testing
```{r}
bar3 <- ggplot(data = df1_1,
               aes(x = yearseason,
                   y = Total,
                   group = year,
                   fill = season)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c(red, GWblue, GWbuff)) +
 scale_x_discrete(labels = df1_1$year) 
  
bar3

# this one probably isn't going anywhere)
bar4 <- ggplot(data = df1_1,
               aes(x = interaction(year, season, lex.order = TRUE),
                   y = Total,
                   group = year,
                   fill = season)) +
  geom_bar(stat = "identity") +
  annotate(geom = "text", x = seq_len(nrow(df1_1)), y = 34,
           label = df1_1$season, size = 4) +
  annotate(geom = "text", y = 32,
           label = unique(df1_1$year), size = 6)
