...
# add rows for missing seasons/UAs, create year, Season, and sort columns [if a yearseason value just specifies the year but not the season (e.g. "2012"), a season is assigned: Fall for 2012, Spring for other single-agenda years (e.g. 2026, published in July 2026)]
df1_1a <- df1_1 %>%
  ...
  mutate(season2 = case_when(season == "" & year == 2012 ~ "Fall",
                             season == "" ~ "Spring",
                             TRUE ~ season)) %>%
...
bar2 <- ggplot(df1_1a_complete,
  ...
  geom_bar(aes(fill = Season), stat = "identity", position = "dodge", width = 0.8) +
  ...
  xlab("\nNote: Only the Fall Agenda is available for 1995, and only one Agenda was published in 2012, 2025, and 2026. Active actions refer \nto rulemakings in a pre-rule, proposed rule or final rule stage as indicated by the Unified Agenda.") +
  ...
  theme_RSC +
  theme(
      plot.title = element_text(hjust = 0.5, vjust = 0, margin = margin(b = 25), size = 20),
      ...
      axis.title.y = element_text(size = 12, angle = 90, vjust = 0.5, hjust = 0.5, margin = margin(r = 0)),
...
suppressWarnings({new_bar <- ggdraw() +
      draw_plot(bar2) +
      draw_image(logo, x = 0.110, y = 0.043, halign = 0, valign = 0, width = 0.2)
